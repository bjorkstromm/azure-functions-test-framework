...
- `Sample.FunctionApp.Durable.Tests` verifies durable metadata discovery, `[DurableClient]` HTTP starter execution, direct activity invocation, sub-orchestrator execution, and provider-driven orchestration completion fully in-process
...
- Support richer durable APIs on the fake path (external events, custom status, management payload helpers)
