...
- **`[BlobInput]` with complex types (e.g. `BlobClient`, `BlockBlobClient`) is not supported by `WithBlobInputContent`.** The `WithBlobInputContent` builder extension injects bytes for the binding, which covers `string`, `byte[]`, `Stream`, `BinaryData`, and `ReadOnlyMemory<byte>` parameter types. Complex SDK client types use a different model-binding-data mechanism. For those types, override the Azure Blob SDK client in DI via `ConfigureServices(services => services.AddSingleton<BlobServiceClient>(fakeBlobServiceClient))`.
