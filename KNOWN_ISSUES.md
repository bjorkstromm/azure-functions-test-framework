...
- **`[DurableClient] DurableTaskClient` fails to resolve in ASP.NET Core mode** — When using `ConfigureFunctionsWebApplication()` with `FunctionsTestHostBuilder`, the `[DurableClient]` parameter was not resolved because the ASP.NET Core path (`GrpcInvocationBridgeStartupFilter.SendInvocationRequestAsync`) does not send synthetic durable binding data in `InputData` (unlike the gRPC-direct path via `FunctionsHttpMessageHandler`). The real `DurableTaskClientConverter` received null/empty `context.Source`, returned `ConversionResult.Failed()`, and `[ConverterFallbackBehavior(Disallow)]` on `DurableClientAttribute` blocked fallback to registered converters. Fixed by registering `FakeDurableTaskClientInputConverter` in DI as the service for the real `DurableTaskClientConverter` type, so `ActivatorUtilities.GetServiceOrCreateInstance` returns our fake converter instead of creating the real one.
