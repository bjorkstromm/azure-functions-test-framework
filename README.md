...
- **ALC type-identity fix** — A MSBuild target shipped in the NuGet `build/` folder (`AzFuncTestFramework_FilterExtensions`) automatically removes overlapping DLLs from `.azurefunctions` after every test-project build, preventing the Worker SDK's isolated `AssemblyLoadContext` from loading duplicate copies of shared assemblies and causing parameter converters to return `null`
