...
- **Durable Functions spike** — `AzureFunctions.TestFramework.Durable` provides fake-backed durable test services (`ConfigureFakeDurableSupport(...)`, native `[DurableClient]` binding support, `FunctionsDurableClientProvider`, fake orchestration runner/client/context types), `Sample.FunctionApp.Durable` isolates a minimal starter/orchestrator/activity app, and `Sample.FunctionApp.Durable.Tests` verifies metadata discovery, `[DurableClient]` starter execution, and provider-driven orchestration completion fully in-process
