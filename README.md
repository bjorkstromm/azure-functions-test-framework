...
Recent durable fake-path follow-up: the previously unsupported Durable query/entity APIs now have matching unit-test coverage in `tests/AzureFunctions.TestFramework.Tests`, and the fake durable client now waits through the entity-scheduled orchestration start race that was causing intermittent sample CI failures.
